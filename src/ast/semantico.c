...
void analise_semantica_funcoes(vector(AST *) declaracoes_funcoes) {
  for (AST **f = cvector_begin(declaracoes_funcoes);
       f != cvector_end(declaracoes_funcoes); f++) {
...
void analise_semantica_verificar_retorno_funcao(enum TipoDados tipo,
  ...
  case Arvore:
    ...
    break;
  case Vetor:
    for (AST **it = cvector_begin(bloco->u.filhos);
         it != cvector_end(bloco->u.filhos); it++) {
  ...
  case Float: {
    ...
    case Int: {
      ...
      constante->token.u.constante.valor.inteiro =
          floatToInt(constante->token.u.constante.valor.flutuante);
...
void analise_semantica_expressao(AST *arvore) {
  if (arvore) {
    ...
    percorrer_arvore_aplicando_funcao(
        arvore, lambda(void, (AST * arv), {
          if (arv->tipo == Folha && arv->token.tipo == Constant &&
              arv->token.u.constante.tipo == Float) {
            achouFloat = true;
          }
        }));
...
void analise_semantica_uso_variavel_nao_declarada_funcao(vector(AST *) funcoes,
                                                         AST *funcao) {
  if (funcao && get_tipo_token(funcao) == DeclarationFunction &&
      funcao->tipo == Vetor) {
    vector(str) ids_funcoes = get_ids_funcoes(funcoes);
    vector(str) ids_declaracoes = get_ids_declaracoes_variaveis(
        cvector_size(funcao->u.filhos) == 4 ? funcao->u.filhos[3]
                                            : funcao->u.filhos[2]);

    percorrer_arvore_aplicando_funcao(
        bloco, lambda(void, (AST * no), {
          if (no && get_tipo_token == Identifier) {
            str id = get_id_id(no);
            for (size_t i = 0; i < cvector_size(ids_funcoes); i++) {
              if (str_eq(id, ids_funcoes[i]))
                return;
...
void analise_semantica_chamada_funcao_tipos_entrada(vector(AST *) funcoes,
                                                    AST *nodo) {
  percorrer_arvore_aplicando_funcao(
      nodo, lambda(void, (AST * no), {
        if (no && get_tipo_no(no) == Arvore &&
            get_tipo_token(no) == FunctionCall) {
          ...
          vector(enum TipoDados) tipos_parametros_fn =
              procurar_tipagem_dos_parametros_funcao(fn);
          vector(enum TipoDados) tipos_parametros_entrada =
              get_lista_parametros(parametros);

          if (cvector_size(tipos_parametros_fn) ==
              cvector_size(tipos_parametros_entrada)) {
            for (size_t i = 0; i < cvector_size(tipos_parametros_fn); i++) {
              if (tipos_parametros_fn[i] != tipos_parametros_entrada[i]) {
                if (tipos_parametros_fn[i] == Int &&
                    tipos_parametros_entrada[i] == Float) {
                  converter_constant_para(parametros->u.filhos[i], Int);
                } else if ((tipos_parametros_fn[i] == Float &&
                            tipos_parametros_entrada[i] == Int)) {
                  converter_constant_para(parametros->u.filhos[i], Float);
                } else {
                  char msg_erro[1000];
                  sprintf(msg_erro,
                          "Tipo de entrada invalida para a funcao '%s'!",
                          str_ptr(id_));
                  exibir_erro(msg_erro);
                }
              }
...
void analise_semantica_atribuicao(vector(AST *) funcoes,
                                  vector(AST *) parametros,
                                  vector(AST *) declaracoes, AST *nodo) {
  if (nodo) {
    vector(str) ids_funcoes = get_ids_funcoes(funcoes);
    percorrer_arvore_aplicando_funcao(
        nodo, lambda(void, (AST * no), {
          if (no && get_tipo_token(no) == Assignment) {
            ...
            case FunctionCall: {
              ...
              if (tipo != tipo_fn) {
                char msg_erro[1000];
                sprintf(msg_erro,
                        "A variavel '%s' tem tipo '%s' mas a funcao '%s' eh do "
                        "tipo '%s'",
                        str_ptr(id), tipo_dado_str(tipo), str_ptr(id_fn),
                        tipo_dado_str(tipo_fn));
                exibir_erro(msg_erro);
              }
            ...
            case Identifier: {
              ...
              if (tipo != tipo_id) {
                char msg_erro[1000];
                sprintf(msg_erro,
                        "A variavel '%s' tem tipo '%s' mas a variavel '%s' eh "
                        "do tipo '%s'",
                        str_ptr(id), tipo_dado_str(tipo), str_ptr(id_id),
                        tipo_dado_str(tipo_id));
                exibir_erro(msg_erro);
              }
            ...
            case ExpressionArithmetic: {
              if (tipo == String) {
                char msg_erro[1000];
                sprintf(msg_erro,
                        "A variavel '%s' tem tipo '%s' e nao pode receber o "
                        "resultado de operacoes aritmeticas",
                        str_ptr(id), tipo_dado_str(tipo));
                exibir_erro(msg_erro);
              }
            } break;
            case Constant: {
              enum TipoDados tipo_c =
                  no->u.arvore.right->token.u.constante.tipo;
